...
def conv_dil_2d(input_, output_dim,
           k_h=5, k_w=5, dilation=2, stddev=0.02, bias=True, pad='SAME',
           name="conv_dil_2d"):
    with tf.variable_scope(name):
        w= tf.get_variable('w', [k_h, k_w, input_.get_shape()[-1], output_dim],
                        initializer=tf.truncated_normal_initializer(stddev=stddev))
        conv = tf.nn.atrous_conv2d(input_, w, dilation, padding=pad)
